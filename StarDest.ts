...
import { GameObject, rareObjectTypes, specialObjectTypes } from './GameObject.js';
...
export class StarDest extends Enemy {
    ...
    die() {
        ...
        if (player.specials.length < 5 && Math.random() < 0.5) {
            const availableSpecialTypes = specialObjectTypes.filter(t => 
                !player.specials.some(item => item.type === t) && 
